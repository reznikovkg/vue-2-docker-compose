...
export default {
  ...
  actions: {
    ...
    sellItem({commit, state}, item) {
      ...
      if (foundItem) {
        commit('removeItem', {item: foundItem});
        commit('addMoney', foundItem.price);
      }
    },
    ...
    calculateWeights({state, commit}, weight) {
      if (state.equipedRod.handleWeight * getRandomInt(1, 3) < weight) {
        commit('removeItem', {item: state.equipedRod});
        return "Rod";
      }

      if (state.equipedReel.handleWeight * getRandomInt(1, 2) < weight) {
        commit('removeItem', {item: state.equipedReel});
        return "Reel";
      }

      if (state.equipedLine.handleWeight < weight) {
        commit('removeItem', {item: state.equipedLine});
        return "Line";
      }
